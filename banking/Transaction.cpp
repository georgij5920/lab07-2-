...
namespace {
struct AccountGuard {
  explicit AccountGuard(Account& account) : account_(&account) { account_->Lock(); }
  ~AccountGuard() { account_->Unlock(); }
...
bool Transaction::Make(Account& from, Account& to, int sum) {
  ...
  AccountGuard guard_from(from);
  AccountGuard guard_to(to);

  Credit(to, sum);
  const bool success = Debit(from, sum + fee_);
  if (!success) {
